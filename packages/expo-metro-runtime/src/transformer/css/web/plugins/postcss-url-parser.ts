...
function getWebpackIgnoreCommentValue(index, nodes, inBetween) {
  ...
  const matched = prevValueNode.value.match(METRO_IGNORE_COMMENT_REGEXP);
...
function parseDeclaration(declaration, key, result, options) {
  ...
  if (declaration.raws && declaration.raws.between) {
    ...
    const matched = declaration.raws.between
      ...
      .match(METRO_IGNORE_COMMENT_REGEXP);
  ...
  if (prevNode && prevNode.type === "comment") {
    const matched = prevNode.text.match(METRO_IGNORE_COMMENT_REGEXP);
  ...
  parsed.walk((valueNode, index, valueNodes): any => {
      ...
      return {
        ...
        async OnceExit() {
          ...
          const resolvedDeclarations = await Promise.all(
            parsedDeclarations.map(async (parsedDeclaration: any) => {
