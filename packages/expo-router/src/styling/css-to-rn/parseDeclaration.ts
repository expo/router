...
export function parseDeclaration(
  ...
  switch (declaration.property) {
    ...
    default: {
      const exhaustiveCheck: never = declaration;
    ...
    case "token":
      switch (tokenOrValue.value.type) {
        ...
        case "close-curly-bracket":
          return undefined;
        default: {
          const exhaustiveCheck: never = tokenOrValue.value;
          throw new Error(`Unhandled case: ${exhaustiveCheck}`);
        }
      }
    default: {
      const exhaustiveCheck: never = tokenOrValue;
      throw new Error(`Unhandled case: ${exhaustiveCheck}`);
    }
  }
}
  ...
  if ("unit" in length) {
    switch (length.unit) {
      ...
      case "cqmax":
        return undefined;
      default: {
        const exhaustiveCheck: never = length.unit;
        throw new Error(`Unhandled case: ${exhaustiveCheck}`);
      }
    }
  } else {
...
function parseFontSize(fontSize: FontSize) {
  switch (fontSize.type) {
    ...
    case "relative":
      return undefined;
    default: {
      const exhaustiveCheck: never = fontSize;
      throw new Error(`Unhandled case: ${exhaustiveCheck}`);
    }
  }
}
